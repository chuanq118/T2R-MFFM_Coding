...
class GPSImageRender(Render):
    def __init__(self, gps_root, ext = 'png'):
    ...
    def render(self, ix, iy):
        ...
        gps = cv2.imread(
            os.path.join(self.gps_root, "{0}.{1}".format(mid, self.ext)),
        ...
        if gps is None:
            print(
                "error in gps:",
                os.path.join(
                    self.gps_root, "{0}.{1}".format(mid, self.ext)),
            )
...
class GPSDataRender(Render):
    ...
    def _sparse_to_dense(self, patchedGPS, length=1024):
        ...
        # gps = cv2.GaussianBlur(gps, (3, 3), 0)
        gps = gps[..., None]
        return gps
